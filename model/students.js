...
exports.index = function (req, res) {
    ...
    async.series({
        one: function (done) {
            test.index("SELECT * FROM (students AS a INNER JOIN classes AS b ON a.`s_class`=b.`c_id`) ORDER BY a.s_id ASC LIMIT " + (parseInt(page) - 1) * pagesize + "," + pagesize, function (list) {
                done(null, list);
            });
        },
        two: function (done) {
            test.index("SELECT COUNT(*) AS total FROM students", function (list) {
                done(null, list);
            });
        }
    }, function (error, result) {
        res.render('students', {
            studentslist: result.one,
...
exports.delete = function (req, res) {
    ...
    async.series({
        one: function (done) {
            test.index("DELETE FROM `students` WHERE `s_id` = " + id, function (list) {
                done(null, list);
            });
        }
    }, function (error, result) {
