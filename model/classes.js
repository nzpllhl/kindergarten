...
exports.index = function (req, res) {
    ...
    async.series({
        one: function (done) {
            test.index("SELECT * FROM (classes AS a INNER JOIN workers AS b ON a.`c_teacher`=b.`w_id`) ORDER BY a.c_id ASC LIMIT " + (parseInt(page) - 1) * pagesize + "," + pagesize, function (list) {
                done(null, list);
            });
        },
        two: function (done) {
            test.index("SELECT COUNT(*) AS total FROM classes", function (list) {
                done(null, list);
            });
        }
    }, function (error, result) {
        res.render('classes', {
            classeslist: result.one,
...
exports.delete = function (req, res) {
    ...
    async.series({
        one: function (done) {
            test.index("DELETE FROM `classes` WHERE `c_id` = " + id, function (list) {
                done(null, list);
            });
        }
    }, function (error, result) {
